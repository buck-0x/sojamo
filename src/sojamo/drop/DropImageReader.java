...
public class DropImageReader implements Runnable {
  ...
  protected DropImageReader(
      final DropEvent theDropEvent
      ) {
  ...
  protected DropImageReader(
      final DropEvent theDropEvent,
      final PImage theDropImageLoader,
  ...
  private void updatePImage(PImage theImage) {
    if (theImage.width > 0 && theImage.height > 0) {
      ...
      System.out.println("### ERROR sojamo.drop. unable to load image from " + _myPath);
    }
  }
...
//    _myDropEvent.process(this);
  ...
  /*
   hasAlpha, ConvertToBufferedImage taken from http://www.marraro.it/archives/12
*/
